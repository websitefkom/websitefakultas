...
export default function ProdiAdminPage() {
  ...
  const handleOpenModal = (id = null) => {
    if (id) {
      ...
      setForm({
        _id: null, nama: "", deskripsi: "", visi: "", misi: [""],
        akreditasi: "", tahun_akreditasi: "",
        dokumenFile: null, dokumenPreview: null, dokumenName: null,
        foto: null, preview: null,
      });
    }
    setShowModal(true);
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === "foto" && files?.length) {
      ...
      setForm({ ...form, dokumenFile: file, dokumenPreview: URL.createObjectURL(file), dokumenName: file.name });
    } else {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      form.misi.forEach((m) => { if (m.trim()) formData.append("misi", m); });
      ...
      setProdiList((prev) =>
        form._id ? prev.map((p) => (p._id === form._id ? { ...p, ...newData } : p)) : [...prev, newData]
      );
      setShowModal(false);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col md:flex-row bg-gray-50 min-h-screen w-full">
        <Sidebar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} userRole="super_admin" />
    ...
    <div className="flex flex-col md:flex-row bg-gray-50 min-h-screen w-full">
      ...
      <div className={`flex-1 transition-all duration-300 ${sidebarOpen ? "md:ml-64" : ""} flex flex-col`}>
        ...
        <main className="w-full max-w-7xl mx-auto px-4 pt-20 pb-24 flex-1">
          {/* Guard: tampilkan block jika tidak punya akses */}
          {!hasAccess ? (
            <UnauthorizedBlock />
          ) : (
            <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
                <h1 className="text-2xl md:text-3xl font-extrabold text-gray-800 flex items-center gap-3">
                  <Info className="w-6 h-6 text-[#2563eb]" />Manajemen Program Studi
                </h1>
                <button
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-6">
                ...
                />
                <select value={filterAkreditasi} onChange={(e) => setFilterAkreditasi(e.target.value)} className="border border-blue-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-400">
                ...
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="border border-blue-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-400">
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-3">
                  <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto border border-gray-200">
                    ...
                    <form onSubmit={handleSave} className="flex flex-col gap-5">
                      ...
                      <div>
                        <label className="text-sm font-semibold text-gray-800 mb-1 block">Dokumen Akreditasi (PDF)</label>
                        <div className="relative w-full">
                          <input type="text" readOnly value={form.dokumenName || ""} placeholder="Belum ada file dipilih" className="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 bg-gray-50 focus:ring-2 focus:ring-blue-400 focus:outline-none transition text-sm" tabIndex={-1} />
                          <label className="absolute top-1/2 right-2 -translate-y-1/2">
                            <input type="file" name="dokumen" accept="application/pdf" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={handleChange} />
                            <span className="inline-block bg-blue-600 text-white text-sm px-3 py-1 rounded-md font-semibold cursor-pointer hover:bg-blue-700 transition">Pilih File</span>
                          </label>
                        </div>
                        <div className="mt-2 flex items-center gap-3">
                          {form.dokumenPreview ? (
                            <>
                              <a href={form.dokumenPreview} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-700 font-medium underline hover:text-blue-800 transition">{form.dokumenName || "Lihat Dokumen"}</a>
                              <button type="button" onClick={clearChosenDokumen} className="text-sm text-red-600 bg-red-100 px-2 py-1 rounded hover:bg-red-200 transition">Hapus</button>
                            </>
                          ) : (
                            <span className="text-sm text-gray-500 italic">Belum memilih dokumen</span>
                          )}
                        </div>
                      </div>
                      <div>
                        <label className="text-sm font-semibold text-gray-800 mb-1 block">Gambar Program Studi</label>
                        <div className="relative w-full">
                          <input type="text" readOnly value={form.foto?.name || ""} placeholder="Belum ada file dipilih" className="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 bg-gray-50 focus:ring-2 focus:ring-blue-400 focus:outline-none transition text-sm" tabIndex={-1} />
                          <label className="absolute top-1/2 right-2 -translate-y-1/2">
                            <input type="file" name="foto" accept="image/*" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={handleChange} />
                            <span className="inline-block bg-blue-600 text-white text-sm px-3 py-1 rounded-md font-semibold cursor-pointer hover:bg-blue-700 transition">Pilih File</span>
                          </label>
                        </div>
                        {form.preview && (
